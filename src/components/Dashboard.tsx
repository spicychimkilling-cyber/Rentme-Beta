      ...
      try {
        ...
        const incoming = localBookings
          ...
          .map((b: any) => ({
            ...
            ownerId: b.ownerId,
            renterId: b.userId,
            renterName: b.renterName || b.userId,
          }));
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-8">
                              <div>
                                ...
                                <div className="text-sm text-gray-600">From: {req.renterName || req.renterId || 'Unknown'}</div>
                                <Badge className={`mt-2 mb-4 w-20 justify-center ${getStatusColor(req.status)}`}>{req.status}</Badge>
                              </div>
                              <div className="flex flex-col gap-2">
                                <div className="text-right text-sm text-gray-600">Nu. {req.totalPrice}</div>
                                <div className="flex gap-2 mt-4">
                                  <Button size="sm" className="w-20" onClick={() => handleApprove(req.id)}>Approve</Button>
                                  <Button size="sm" variant="outline" className="w-20" onClick={() => handleReject(req.id)}>Reject</Button>
                                </div>
                              </div>
                            </div>
